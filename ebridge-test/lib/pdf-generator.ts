...
export async function generateGradePDF(
  ...
  return new Promise((resolve, reject) => {
    ...
    doc.fillColor('white').fontSize(20).text('E-Bridge Test Results', MARGIN, 16)
    ...
    doc.fillColor(DARK).fontSize(13).text(candidateInfo.fullName || 'Thí sinh', MARGIN + 10, y + 8)
    doc.fontSize(9).fillColor(GRAY)
      .text(
        `SĐT: ${candidateInfo.phone || '—'} · Email: ${candidateInfo.email || '—'} · Ngày sinh: ${candidateInfo.dob || '—'} · Quê quán: ${candidateInfo.hometown || '—'}`,
        MARGIN + 10, y + 28
      )
    y += 60
    ...
    const PARTS = [
      ...
      {
        key: 'part5', icon: '5', title: `Writing Reflection (${grades.part5?.wordCount ?? 0} từ)`,
        criteria: [
    ...
    for (const part of PARTS) {
      ...
      // Part header
      sectionHeader(doc, MARGIN, y, CONTENT_W, `Part ${part.icon} — ${part.title}    ${part.total ?? 0}/20 điểm`)
      y += 26
      ...
      // Feedback
      if (part.feedback) {
        doc.fillColor(GRAY).fontSize(8).text(`Nhận xét: ${part.feedback}`, MARGIN, y, { width: CONTENT_W })
    ...
    if (writing) {
      ...
      sectionHeader(doc, MARGIN, y, CONTENT_W, 'Bài viết (Part 5)')
      y += 26
    ...
    sectionHeader(doc, MARGIN, y, CONTENT_W, 'Nhận xét tổng quan')
    y += 26
    ...
    doc.fillColor(GREEN).fontSize(9).text('Điểm mạnh', MARGIN + 6, y + 4)
    doc.fillColor(DARK).fontSize(8)
      .text(grades.overall?.strengths || '—', MARGIN + 6, y + 16, { width: half - 12, height: 40 })

    doc.fillColor(AMBER).fontSize(9).text('Cần cải thiện', MARGIN + half + 18, y + 4)
    doc.fillColor(DARK).fontSize(8)
    ...
    doc.fillColor(GRAY).fontSize(7)
      .text(
        `E-Bridge Program · Kết quả được tạo bởi Gemini AI · ${new Date().toLocaleDateString('vi-VN', { timeZone: 'Asia/Ho_Chi_Minh' })}`,
